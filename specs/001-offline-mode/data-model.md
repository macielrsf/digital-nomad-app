...
const realmConfig: Realm.Configuration = {
  schema: [RealmOfflineCity, RealmSyncAction, RealmOfflinePreferences, RealmDownloadJob],
  schemaVersion: 2,
...
```typescript
realm.objects('OfflineCity')
  .sorted('lastAccessedAt', true); // descending
```

**Get pending sync actions (FIFO order)**:
```typescript
realm.objects('SyncAction')
